...
class LayerNorm2d(nn.LayerNorm):
    def forward(self, x: torch.Tensor):
        input_dtype = x.dtype
        x = x.to(self.weight.dtype) # Ensure the input is the same dtype as the weight for layer norm
        x = x.permute(0, 2, 3, 1)
        x = nn.functional.layer_norm(x, self.normalized_shape, self.weight, self.bias, self.eps)
        x = x.permute(0, 3, 1, 2)
        return x.to(input_dtype) # Return to the original dtype
...
class SwiGLU(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.SiLU, drop=0., channels_first=False,
