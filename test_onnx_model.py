...
def detect(args):
    ...
    result = model.run(None, {"input.1": np.zeros((1, 3, 384, 640), dtype=np.float32)})
    ...
    for i, (path, img, img_det, vid_cap,shapes) in tqdm(enumerate(dataset),total = len(dataset)):
        ...
        output = model.run(None, {"input.1": img.numpy()})
        da_seg_out,ll_seg_out= output[0], output[1]
        da_seg_out = torch.from_numpy(da_seg_out)
        ll_seg_out = torch.from_numpy(ll_seg_out)
